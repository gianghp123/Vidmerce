...
import { getImageUploadUrl } from "../services/asset.get";
import { uploadImageToS3, confirmAssetUpload } from "../services/asset.action";
    ...
    try {
      for (const file of filesToUpload) {
        ...
        const { uploadUrl } = uploadData;
         ...
         // After all files are uploaded, confirm the asset upload
         if (file === filesToUpload[filesToUpload.length - 1]) {
           await confirmAssetUpload(assetId, {
             name: asset.name,
             price: asset.price,
             productUrl: asset.productUrl,
             fileKeys: filesToUpload.map(file => file.name) // Using file names as keys for now
           });
         }
       }
